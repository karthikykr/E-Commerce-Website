...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const productsResponse = await api.get('/api/product?page=1&limit=1');
      const totalProducts = productsResponse.data.pagination?.total || 0;

      const activeProductsResponse = await api.get('/api/product?status=active&page=1&limit=1');
      const activeProducts = activeProductsResponse.data.pagination?.total || 0;

      const categoriesResponse = await api.get('/api/category');
      const totalCategories = categoriesResponse.data.count || 0;

      const recentResponse = await api.get('/api/product?page=1&limit=5');
      let recentData = recentResponse.data.product || [];
      recentData = recentData.sort(
        (a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(),
      );
      setRecentProducts(recentData);

      const totalRevenue = recentData.reduce((sum: number, p: any) => sum + p.price * p.stock, 0);
  ...
  return (
    <AdminLayout title="Dashboard">
      <div className="space-y-6">
        ...
        {/* Stats Cards - Adapted to Products/Categories */}
        {loading ? (
        ...
        {/* Recent Activity - Adapted to Recent Products */}
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
