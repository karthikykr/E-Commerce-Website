...
import { clearError } from '@/redux/slices/authSlice'; // From your auth slice
...
export function AdminLayout({ children, title = 'Admin Dashboard' }: AdminLayoutProps) {
  ...
  const { user } = useSelector((state: RootState) => state.auth); // Use Redux instead of useAuth

  const handleLogout = () => {
    // Clear auth state
    localStorage.removeItem('token'); // Assuming token is stored here (as in your login flow)
    dispatch(clearError()); // Clear any errors from Redux
    // If you add a full logout action to authSlice later, dispatch it here
      ...
      >
        ...
        <div className="p-4 border-b border-gray-700">
          <div className="flex items-center">
            ...
            {sidebarOpen && <span className="ml-3 text-lg font-semibold">Admin Panel</span>}
          </div>
        </div>
