  ...
  const fetchFeaturedProducts = async () => {
    try {
      const response = await fetch(
        'http://localhost:5000/api/featured-products/homepage'
      );
      if (response.ok) {
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-gray-200 rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-4 animate-pulse"
              >
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-10">
          ...
          {featuredProducts.heroMain.map((product, index) => (
            ...
            >
              <div
                className={`bg-white rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 transform ${product.rotation} hover:${product.hoverRotation} transition-all duration-500 hover:shadow-2xl border border-orange-100`}
              >
                <div
                  className={`w-full h-32 sm:h-40 bg-gradient-to-br ${product.backgroundColor} rounded-lg sm:rounded-xl flex items-center justify-center mb-3 sm:mb-4 shadow-inner relative overflow-hidden`}
                >
                  {product.image?.url ? (
                    <img
                    ...
                    <span
                      className={`text-3xl sm:text-4xl animate-${product.animation}`}
                    >
                      {product.emoji}
                    </span>
                  )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
          {featuredProducts.heroBottom.map((product) => (
            ...
            >
              <div
                className={`w-full h-20 sm:h-24 bg-gradient-to-br ${product.backgroundColor} rounded-md sm:rounded-lg flex items-center justify-center mb-2 sm:mb-3 shadow-inner relative`}
              >
                {product.image?.url ? (
                  <img
                    src={`http://localhost:5000${product.image.url}`}
                  ...
                  <span
                    className={`text-xl sm:text-2xl animate-${product.animation}`}
                  >
                    {product.emoji}
                  </span>
                )}
              ...
              <div className="flex items-center justify-center space-x-1 mt-1">
                <p className="text-orange-600 font-bold text-xs text-center">
                  ₹{product.price}
                </p>
                {product.originalPrice && (
                  <span className="text-gray-400 line-through text-xs">
