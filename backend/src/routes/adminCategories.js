...
router.get('/', adminAuth, logAdminAction('VIEW_CATEGORIES'), async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 20, 
...
], logAdminAction('CREATE_CATEGORY'), async (req, res) => {
  try {
...
router.put('/:id', [
  ...
  body('sortOrder').optional().isInt({ min: 0 }).withMessage('Sort order must be a non-negative integer')
], logAdminAction('UPDATE_CATEGORY'), async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
// @access  Private (Admin)
router.delete('/:id', adminAuth, logAdminAction('DELETE_CATEGORY'), async (req, res) => {
  try {
    const category = await Category.findById(req.params.id);
...
// @access  Private (Admin)
router.put('/:id/toggle-status', adminAuth, logAdminAction('TOGGLE_CATEGORY_STATUS'), async (req, res) => {
  try {
    const category = await Category.findById(req.params.id);
...
], logAdminAction('REORDER_CATEGORIES'), async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
