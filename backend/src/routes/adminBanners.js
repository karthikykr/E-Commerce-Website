...
// @access  Private (Admin)
router.get('/:id', adminAuth, logAdminAction('VIEW_BANNER'), async (req, res) => {
  try {
...
router.post('/', 
  ...
  [
    ...
    body('position').isIn(['hero', 'top', 'middle', 'bottom', 'sidebar']).withMessage('Invalid position'),
...
router.put('/:id',
  ...
  [
    ...
    body('position').optional().isIn(['hero', 'top', 'middle', 'bottom', 'sidebar']).withMessage('Invalid position'),
    body('displayOrder').optional().isInt({ min: 0 }).withMessage('Display order must be a non-negative integer'),
    body('startDate').optional().isISO8601().withMessage('Start date must be a valid date'),
    body('endDate').optional().isISO8601().withMessage('End date must be a valid date')
  ],
  ...
  async (req, res) => {
    try {
      ...
      if (linkOpenInNewTab !== undefined) banner.link.openInNewTab = linkOpenInNewTab === 'true';

      // Update settings
      if (backgroundColor !== undefined) banner.settings.backgroundColor = backgroundColor;
      if (textColor !== undefined) banner.settings.textColor = textColor;
      if (overlayOpacity !== undefined) banner.settings.overlayOpacity = parseFloat(overlayOpacity);
      if (textAlignment !== undefined) banner.settings.textAlignment = textAlignment;
      if (animation !== undefined) banner.settings.animation = animation;
      if (autoPlay !== undefined) banner.settings.autoPlay = autoPlay === 'true';
      ...
      if (req.file) {
        ...
        if (banner.image.filename) {
          deleteUploadedFiles([{
            path: path.join(__dirname, '../../uploads/banners', banner.image.filename),
...
// @desc    Delete banner
// @access  Private (Admin)
router.delete('/:id', adminAuth, logAdminAction('DELETE_BANNER'), async (req, res) => {
  try {
    const banner = await Banner.findById(req.params.id);
    if (!banner) {
      return res.status(404).json({
...
// @access  Private (Admin)
router.put('/:id/toggle', adminAuth, logAdminAction('TOGGLE_BANNER'), async (req, res) => {
  try {
    const banner = await Banner.findById(req.params.id);
    if (!banner) {
      return res.status(404).json({
...
router.put('/reorder', [
  ...
  body('banners.*.displayOrder').isInt({ min: 0 }).withMessage('Display order must be a non-negative integer')
], logAdminAction('REORDER_BANNERS'), async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
