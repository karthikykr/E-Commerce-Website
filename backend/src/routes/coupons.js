...
router.post('/apply', [
  ...
  try {
    ...
    order.total = order.subtotal + order.shippingCost + order.tax - discountAmount;
    await order.save();
...
router.post('/admin', [
  ...
  body('minOrderAmount').optional().isFloat({ min: 0 }).withMessage('Min order amount must be non-negative')
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
