...
notificationSchema.statics.createOrderNotification = async function(userId, orderId, type, customMessage) {
  const messages = {
  ...
  const notification = new this({
    ...
    message: customMessage || messages[type] || 'Your order status has been updated.',
    type: type.includes('failed') || type.includes('cancelled') ? 'error' : 
          type.includes('delivered') || type.includes('received') ? 'success' : 'info',
...
notificationSchema.statics.createProductNotification = async function(userId, productId, type, customMessage) {
  const messages = {
  ...
  const notification = new this({
    ...
    type: type === 'price_drop' || type === 'back_in_stock' ? 'success' : 'info',
...
notificationSchema.statics.createPromotionalNotification = async function(userId, title, message, actionUrl, metadata) {
  const notification = new this({
    user: userId,
    title: title,
    message: message,
...
notificationSchema.statics.sendBulkNotifications = async function(userIds, notificationData) {
